...
import type { MenuFormData, MenuStatus } from '@/types';

interface AddMenuModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddMenu: (menuData: MenuFormData) => void;
}

export default function AddMenuModal({ isOpen, onClose, onAddMenu }: AddMenuModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.name && formData.date && formData.deadline) {
      onAddMenu(formData);
      handleClose();
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-bold text-main-text">Add New Menu</h2>
          <button
            onClick={handleClose}
