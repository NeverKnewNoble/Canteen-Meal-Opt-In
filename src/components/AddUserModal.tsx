...
interface AddUserModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddUser: (userData: { name: string; department: string }) => void;
  departments: string[];
}

export default function AddUserModal({ isOpen, onClose, onAddUser, departments }: AddUserModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name.trim() && department) {
      onAddUser({ name: name.trim(), department });
      setName('');
      setDepartment(departments[0] || '');
      onClose();
    }
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 border-2 border-gray-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-main-text">Add New User</h2>
          </div>
          <button
            onClick={handleClose}
